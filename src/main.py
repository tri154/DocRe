...
def run_training(cfg, no_tqdm=False):
    ...
    trainer.train(cfg.num_epoch, cfg.train_batch_size, run_both=False, no_tqdm=no_tqdm)
    trainer.train(cfg.num_epoch, cfg.train_batch_size, run_both=True, no_tqdm=no_tqdm)
    return 0
    # trainer.debug()
    # tester.test(model, dataset='dev', run_both=True)
